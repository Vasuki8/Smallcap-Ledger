"""One-time JM Small Cap TER recovery; nightly AMC expense collection remains active."""
...
UPGRADE_KEY = "source_upgrade_jm-ter-v1"
FAMILY = amc_expenses.JM_FAMILY


def run():
    db.init()
    if db.setting(UPGRADE_KEY, False):
        print("JM Small Cap TER recovery already applied; nightly collection remains active.")
        return True

    result = amc_expenses.jm()
    ...
    if success:
        ...
        success = (
            len(days) == 1
            and sources == {amc_expenses.JM_TER_API}
            and len(hashes) == 1
            and bool(next(iter(hashes)))
        )
    if not success:
        raise RuntimeError("JM TER recovery did not produce a recent exact four-metric plan pair")
    ...
    print(result)
    print("JM_TER_SOURCE", next(iter(sources)))
    print("JM_TER_HASH", next(iter(hashes)))
    return True
