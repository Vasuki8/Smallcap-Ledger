...
def run():
    ...
    if amc_reports.PARSER_VERSION=='amc-reports-2026-09-v129':
        family='Bandhan Small Cap Fund';attempted=0;current=False;previous=False
        try:
            from tracker import amc_discovery
            for found_family,url,title in amc_discovery.discover('Bandhan'):
                attempted+=1
                try:
                    amc_discovery.store_report('Bandhan',found_family,url,title)
                except Exception as exc:
                    print(f"::warning::Bandhan portfolio candidate: {(str(exc) or type(exc).__name__).splitlines()[0][:250]}",flush=True)
            snapshots=db.rows("""SELECT p.as_of,p.complete,COUNT(h.id) positions,SUM(h.weight) weight
              FROM portfolios p LEFT JOIN holdings h ON h.snapshot_id=p.id
              WHERE p.family=? AND p.as_of IN ('2026-08-31','2026-07-31')
              GROUP BY p.id ORDER BY p.as_of DESC,p.id DESC""",(family,))
            ...
            for snap in snapshots:by_day.setdefault(snap['as_of'],snap)
            latest=by_day.get('2026-08-31');prior=by_day.get('2026-07-31')
            current=bool(latest and not latest['complete'] and latest['positions']>=260)
            previous=bool(prior and not prior['complete'] and prior['positions']>=260)
            detail=lambda x:'none' if not x else f'{x["as_of"]}, {x["positions"]} positions, {float(x["weight"] or 0):.6f}% known numeric weight, complete={x["complete"]}'
            if current and previous:
                print(f'Bandhan current and prior censored partial portfolios verified: {detail(latest)}; prior {detail(prior)}',flush=True)
            else:
                print(f'::warning::Bandhan partial portfolio recovery incomplete; latest={detail(latest)}; prior={detail(prior)}; attempted={attempted}',flush=True)
        except Exception as exc:
            print(f"::warning::Bandhan v129 discovery: {(str(exc) or type(exc).__name__).splitlines()[0][:300]}",flush=True)
        ok.append(current and previous)
