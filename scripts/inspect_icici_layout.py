"""Temporary read-only diagnostic for retained ICICI Prudential Small Cap factsheet pages."""
...
def main():
    ...
    if not rows:
        print('ICICI_LAYOUT_DIAGNOSTIC=no factsheet versions',flush=True);return
    materialize_hashes([r['hash'] for r in rows])
    pattern=re.compile(r'ICICI\s+Prudential\s+Small\s*Cap\s+Fund',re.I)
    for row in rows:
        ...
        for index in wanted[:8]:
            ...
            payload={
              ...
              'text':text[:14000],
              'layout':layout[:14000] if layout!=text else '',
            }
            print('ICICI_LAYOUT_PAGE='+json.dumps(payload,ensure_ascii=False,separators=(',',':')),flush=True)
