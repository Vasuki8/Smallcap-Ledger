"""Read-only production probe for PGIM India and quant communication sources."""
...
SOURCES=(
    ...
    ("Quant","https://www.quantmutual.com/downloads/investment_outlook"),
)
...
def main():
    for amc,url in SOURCES:
        ...
        try:
            ...
            print("COMM5_TITLE",amc,repr(title[:350]),flush=True)
            print("COMM5_DATE",amc,url,explicit_publication_date(body,typ,url),flush=True)
            print("COMM5_TEXT",amc,url,repr(text[:5000]),flush=True)
            ...
            for script in soup.find_all("script"):
                ...
                low=raw.lower()
                if any(k in low for k in ("ceo letters","outlooks & economy","predictive analytics","vlrt outlook","investment outlook")):
                    print("COMM5_SCRIPT",amc,url,repr(raw[:8000]),flush=True)
        except Exception as exc:
            print("COMM5_ERROR",amc,url,(str(exc) or type(exc).__name__).splitlines()[0][:1200],flush=True)
