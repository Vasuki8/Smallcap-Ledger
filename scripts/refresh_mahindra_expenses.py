"""One-time Mahindra Manulife Small Cap TER recovery; nightly collection remains active."""
...
UPGRADE_KEY = "source_upgrade_mahindra-ter-v1"
FAMILY = amc_expenses.MAHINDRA_FAMILY


def run():
    db.init()
    if db.setting(UPGRADE_KEY, False):
        print("Mahindra Manulife TER recovery already applied; nightly collection remains active.")
        return True

    result = amc_expenses.mahindra()
    ...
    if success:
        ...
        success = (
            ...
            and next(iter(sources)).startswith(
                "https://www.mahindramanulife.com/uploads/download/"
            )
            and len(hashes) == 1
    ...
    if not success:
        raise RuntimeError(
            "Mahindra Manulife TER recovery did not produce a recent exact four-metric plan pair"
        )
    ...
    print(result)
    print("MAHINDRA_TER_SOURCE", next(iter(sources)))
    print("MAHINDRA_TER_HASH", next(iter(hashes)))
    return True
