...
class SbiSundaramCommunicationTests(unittest.TestCase):
    ...
    def test_sources_are_registered_and_audit_recognizes_them(self):
        ...
        self.assertEqual(len(rows[upgrade.SBI_FAMILY]["registered_communication_sources"]),1)
        self.assertEqual(len(rows[upgrade.SUNDARAM_FAMILY]["registered_communication_sources"]),1)

    def test_sbi_exact_page_is_market_view_with_source_explicit_date(self):
        body=(b"<html><body><h1>2026 Outlook</h1>"
              b"<p>Published 8th January 2026</p>"
              b"<h2>Executive Summary</h2><h2>Equity Outlook</h2>"
        ...
        self.assertEqual(
            ...
            "2026-01-08")
        unrelated="https://www.sbimf.com/learn-about-mutual-funds/other-page"
        self.assertIsNone(
            disclosures.explicit_publication_date(body,"text/html",unrelated))
    ...
    def test_sbi_generic_ingest_retains_exact_page(self):
        body=(b"<html><body><h1>2026 Outlook</h1>"
              ...
              b"<h2>Fixed Income Outlook</h2>"
              b"<p>Rajeev Radhakrishnan, CFA, CIO - Fixed Income</p></body></html>")
        with patch("tracker.disclosures.can_crawl",return_value=None),              patch("tracker.disclosures.fetch",
    ...
    def test_sundaram_direct_outlook_pdf_is_market_view_without_inferred_day(self):
        pdf=b"%PDF-1.6 "+b"Sundaram Mutual Fund Outlook "*5000
        with patch("tracker.disclosures.can_crawl",return_value=None),              patch("tracker.disclosures.fetch",
                   return_value=self.archived(pdf,"application/pdf")),              patch("tracker.amc_reports.extract",return_value=0):
    ...
    def test_recovery_gate_is_idempotent_and_requires_exact_evidence(self):
        ...
        sundaram=b"%PDF-1.6 "+b"Sundaram Outlook "*7000
