...
def parse_sheet(rows,formats,family):
    ...
    vc=col(lambda v:re.search(r'market|mkt|fair',v) and re.search(r'value|\bval\b',v))
    ...
    for ri,row in enumerate(rows[hi+1:],hi+1):
        ...
        try:w=weight(ri,row);v=numeric(row[vc])
            ...
            if family=='Bandhan Small Cap Fund' and valid_isin and str(row[wc] or '').strip()==chr(36):
                censored.append({'name':name,'isin':isin,'quantity':None if qc is None or qc>=len(row) else row[qc],
                                 'marker':chr(36),'meaning':'Less Than 0.01% of NAV'})
