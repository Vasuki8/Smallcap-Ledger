...
def absl_zip(content,family,url,h):
    """Parse ABSL's official monthly portfolio ZIP without extracting paths."""
    import io,zipfile
    ...
    with zipfile.ZipFile(io.BytesIO(content)) as z:
        ...
        if len(entries)>250 or sum(i.file_size for i in entries)>150*1024*1024:
            raise ValueError('Oversized ABSL portfolio ZIP')
        spreadsheets=[]
        for i in entries:
            p=PurePosixPath(i.filename)
            if p.is_absolute() or '..' in p.parts or '\\' in i.filename or i.flag_bits&1:
                raise ValueError('Unsupported ABSL portfolio ZIP entry')
