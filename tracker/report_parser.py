...
def union_complete_portfolio(text):
    ...
    for line in lines[start+1:end]:
        ...
        if letters and label.upper()==label:continue
        if not 0<weight<20:return None
        positions.append({'name':label,'isin':None,'sector':None,'weight':weight,'asset_type':'Equity'})
...
def hsbc_complete_portfolio(text):
    ...
    owned=owns_page(normalized,'HSBC Small Cap Fund')
    if not owned:
...
def layout_aum(text,family,day):
    ...
    return out
,normalized,re.I))
        portfolio_heading=bool(re.search(r'(?m)^\s*Portfolio\s*    day=dated(total_match.group(1));grand=float(total_match.group(2))
    if not day or abs(grand-100)>.03:return None
    start=re.search(r'Issuer\s+Market Cap/\s*\n?\s*Ratings\s+%\s*to\s*Net\s*Assets',normalized,re.I)
    if not start:return None
    section=normalized[start.end():total_match.start()]
